...
def reverse_string(str)
    return "" if str == "" #base condition
    #so I need to shorten the string by one each time
    return str[-1] + reverse_string(str[0...-1]) #nice!
end
...
def flatten(data)
    return [data] if !data.is_a?(Array) #our goal is to get rid of all of the brackets, and then eventually have a list of numbers that we put into an array
    flattened = []
    data.each do |ele|
...
# flattened = []
# flattened = [] + flatten(1) -> [1]
# flattened = [1] + flatten(2) -> [1] + [2] = [1,2]
# flattened = [1,2] + flatten([3,4])

#complicated, but I think I undersatnd enough for now at least


# return 0 if array.empty?
#     return array[0] + sum_array(array[1..-1]) #keeps on calling sum_array on the shortened array
# end
